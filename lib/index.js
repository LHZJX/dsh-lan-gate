...
async function loadStateFile() {
  const fallback = { record: null, setAt: undefined, bind: { mode: 'all' }, tls: { enabled: false, port: TLS_DEFAULT_PORT } }
  try {
...
function tlsDesired(state) {
  ...
  const host = state.boundHost
  if (host === '127.0.0.1') return null // local 模式没有局域网面
  return host // '0.0.0.0' 或具体局域网 IP
}
...
function lanTlsUrls(state) {
  ...
  if (state.boundHost === '0.0.0.0') return detectAddrs().map((entry) => `https://${entry.address}:${String(port)}`)
  if (state.boundHost === '127.0.0.1') return []
  return [`https://${state.boundHost}:${String(port)}`]
}

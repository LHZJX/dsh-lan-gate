...
/** 保证证书存在且可用;缺失/损坏时按当前网卡 SAN 重新生成。 */
async function ensureTlsCert(state) {
  ...
  if (existing !== null) {
    try {
      ...
      if (parsed.subject.includes('CN=dsh-lan-gate')) return existing
    } catch {
      /* 损坏 → 重新生成 */
    }
  }
  ...
  const pem = buildSelfSignedCert(sans)
...
async function syncTlsServer(ctx, state) {
  ...
  if (state.tlsServer) return { ok: true, active: true }

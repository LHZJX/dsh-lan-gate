...
import { X509Certificate, createSign, generateKeyPairSync, randomBytes, scryptSync, timingSafeEqual } from 'node:crypto'
...
// ── TLS(局域网 HTTPS)───────────────────────────────────────────────
/** 默认 HTTPS 监听端口(避开 443/3080,减少端口冲突)。 */
export const TLS_DEFAULT_PORT = 3443
/** $DSH_HOME 下保存自动生成的自签证书的子目录。 */
const TLS_DIR = 'lan-gate-tls'
const TLS_CERT_FILE = 'cert.pem'
const TLS_KEY_FILE = 'key.pem'
/** 自签证书有效期:10 年(避免频繁换证导致手机要反复信任)。 */
const TLS_VALIDITY_DAYS = 3650

/** Mobile-only UI prefix: an inert static shell served to every caller. */
export const MOBILE_PREFIX = '/mobile'

/** Whitelisted mobile assets under /mobile (name -> content type). */
const MOBILE_ASSETS = {
...
/** IPv6 首段是否属于链路本地(fe80::/10)。链路本地地址依赖网卡 zone,浏览器/
 *  ArkWeb 无法携带 zone 访问,故不列入可探测/可绑定的地址。 */
function isLinkLocalIpv6(address) {
  const first = Number.parseInt(String(address).split(':')[0], 16)
  return !Number.isNaN(first) && first >= 0xfe80 && first <= 0xfebf
}

/** 探测本机可被访问的单播网卡地址:[{ name, address, family }]。
 *  IPv4 全部(排除 internal);IPv6 排除 internal(::1)与链路本地(fe80::/10)。
 *  IPv4 在前保持既有顺序与默认行为,IPv6 追加在后。 */
function detectAddrs() {
  ...
  for (const [name, ifaces] of Object.entries(networkInterfaces())) {
    for (const iface of ifaces ?? []) {
      ...
      out.push({ name, address: iface.address, family: 'IPv4' })
    }
  }
  for (const [name, ifaces] of Object.entries(networkInterfaces())) {
    for (const iface of ifaces ?? []) {
      if (!iface || iface.internal || iface.family !== 'IPv6') continue
      if (isLinkLocalIpv6(iface.address)) continue
      if (seen.has(iface.address)) continue
      seen.add(iface.address)
      out.push({ name, address: iface.address, family: 'IPv6' })
    }
  }
  return out
}
...
function createState() {
  return {
    ...
    tls: { enabled: false, port: TLS_DEFAULT_PORT }, // 持久化的 HTTPS 意图
    tlsServer: undefined, // { server, host, port, close } — 运行中的 HTTPS 监听
    http: { enabled: true }, // 持久化的“局域网明文 HTTP”意图;false=核心服务只绑回环
...
async function loadStateFile() {
  const fallback = {
    ...
    setAt: undefined,
    bind: { mode: 'all' },
    tls: { enabled: false, port: TLS_DEFAULT_PORT },
    http: { enabled: true }
  }
  try {
    ...
    let tls = { enabled: false, port: TLS_DEFAULT_PORT }
    const storedTls = parsed?.tls
    if (storedTls && typeof storedTls === 'object') {
      const port = Number(storedTls.port)
      tls = {
        enabled: storedTls.enabled === true,
        port: Number.isInteger(port) && port > 0 && port <= 65535 ? port : TLS_DEFAULT_PORT
      }
    }
    return {
      record: password,
      ...
      bind,
      tls,
      http: { enabled: !(parsed?.http && parsed.http.enabled === false) }
    }
  } catch (error) {
...
async function saveStateFile(state) {
  ...
  const payload = {
    ...
    bind: state.bind,
    tls: {
      enabled: state.tls?.enabled === true,
      port: state.tls?.port ?? TLS_DEFAULT_PORT
    },
    http: {
      enabled: state.http?.enabled !== false
    }
  }
...
// ── TLS:自签证书 + HTTPS 监听 ──────────────────────────────────────
//
// 不依赖 openssl:用 node:crypto 直接产出 RSA-2048 自签证书(CA:TRUE +
// subjectAltName 覆盖本机所有可访问的单播 IPv4/IPv6 与回环),写入 $DSH_HOME/lan-gate-tls/。
// HTTPS 监听与 loopback twin 同构:镜像 webServer 的分发 + 挂同一套网关。

/** OIDs used by the self-signed certificate. */
const OID = {
...
/** Minimal DER encoder (enough for one self-signed X.509). */
function derLen(n) {
  if (n < 0x80) return [n]
  const bytes = []
  let v = n
...
function der(tag, body) {
  return Buffer.concat([Buffer.from([tag]), Buffer.from(derLen(body.length)), body])
}
function derInt(n) {
  const bytes = []
  let v = n
...
function derUtf8(s) {
  return der(0x0c, Buffer.from(s, 'utf8'))
}
...
function derName(cn) {
  return derSeq(derSet(derSeq(derOid(OID.commonName), derUtf8(cn))))
}
function derOctet(body) {
  return der(0x04, body)
}

/** 4-byte IPv4 buffer or null. */
...
/** 把任意 IPv6 字面量展开为小写完整形式(去掉 ::),无法解析返回 null。
 *  用于证书 SAN 生成与「SAN 是否漂移」的规范化比较(证书里打印出来的是完整展开形式)。 */
function expandIpv6(input) {
  ...
  if (halves.length === 2) {
    ...
    left = left.concat(new Array(fill).fill('0'), right)
  }
  if (left.length !== 8) return null
  const groups = []
  for (const part of left) {
    if (!/^[0-9a-f]{1,4}$/.test(part)) return null
    groups.push(part.padStart(4, '0'))
  }
  return groups.join(':')
}

/** 16-byte IPv6 buffer or null。 */
function ipv6Bytes(s) {
  const expanded = expandIpv6(s)
  if (expanded === null) return null
  const out = []
  for (const group of expanded.split(':')) {
    out.push(Number.parseInt(group.slice(0, 2), 16), Number.parseInt(group.slice(2, 4), 16))
  }
  return Buffer.from(out)
}

/** 生成自签证书 {key, cert}(PEM)。sans:域名/IP 字符串数组。 */
function buildSelfSignedCert(sans) {
  ...
  serial[0] &= 0x7f
  const now = Date.now()
  const notBefore = new Date(now - 24 * 3600e3)
  const notAfter = new Date(now + TLS_VALIDITY_DAYS * 86400e3)

  const critical = der(0x01, Buffer.from([0xff])) // BOOLEAN TRUE
  // basicConstraints: cA=TRUE(可作 CA 信任锚),keyUsage: digitalSignature|keyEncipherment|keyCertSign|cRLSign
  const extBasic = derSeq(derOid(OID.basicConstraints), critical, derOctet(derSeq(critical)))
  const extKeyUsage = derSeq(derOid(OID.keyUsage), critical, derOctet(der(0x03, Buffer.from([0x01, 0xa6]))))
  const names = []
  for (const s of sans) {
    const ip4 = ipv4Bytes(s)
    if (ip4 !== null) names.push(der(0x87, ip4))
    else {
      const ip6 = ipv6Bytes(s)
      if (ip6 !== null) names.push(der(0x87, ip6))
      else names.push(der(0x82, Buffer.from(String(s), 'ascii'))) // dNSName
    }
  }
  const extSan = derSeq(derOid(OID.subjectAltName), derOctet(derSeq(...names)))

  const algId = derSeq(derOid(OID.sha256WithRSA), der(0x05, Buffer.alloc(0))) // sha256WithRSA + NULL
  const tbs = derSeq(
    der(0xa0, derInt(2)), // version v3
    derBigInt(serial),
    algId,
    derName('dsh-lan-gate'),
    derSeq(derUtcTime(notBefore), derUtcTime(notAfter)),
    derName('dsh-lan-gate'),
    spkiDer, // SubjectPublicKeyInfo 本身是 SEQUENCE,整段嵌入
    der(0xa3, derSeq(extBasic, extKeyUsage, extSan)) // [3] extensions
  )
  const signature = createSign('RSA-SHA256').update(tbs).sign(privateKey)
  const certDer = derSeq(tbs, algId, der(0x03, Buffer.concat([Buffer.from([0]), signature])))
  const b64 = certDer.toString('base64')
  const certPem = `-----BEGIN CERTIFICATE-----\n${b64.match(/.{1,64}/g).join('\n')}\n-----END CERTIFICATE-----\n`
  return { key: keyPem, cert: certPem }
}

/** Absolute paths of the auto-managed cert files. */
function tlsAssetPaths() {
  return {
    dir: join(dshHome(), TLS_DIR),
    cert: join(dshHome(), TLS_DIR, TLS_CERT_FILE),
    key: join(dshHome(), TLS_DIR, TLS_KEY_FILE)
  }
}

/** 从证书 PEM 读出 SAN 中的条目集合(DNS 名 + IP;IPv6 统一展开为完整小写形式)。 */
function sanEntriesOf(certText) {
  const x = new X509Certificate(certText)
  // X509Certificate 类的属性是 subjectAltName(驼峰);getPeerCertificate() 才是全小写
  const raw = x.subjectAltName ?? x.subjectaltname ?? ''
  const set = new Set()
  for (const part of String(raw).split(',')) {
    const m = /^(?:DNS|IP Address):\s*(.+)$/.exec(part.trim())
    if (!m) continue
    let value = m[1].trim()
    if (value.includes(':')) value = expandIpv6(value) ?? value
    set.add(value)
  }
  return set
}

/** 当前应写入证书的 SAN 条目(回环 + 全部探测到的网卡 + 绑定目标)。
 *  IPv6 一律展开为完整小写形式,与 sanEntriesOf 的解析结果一致,避免误判漂移。 */
function desiredSanEntries(state) {
  const sans = ['localhost', '127.0.0.1']
  const add = (address) => {
    const canonical = address.includes(':') ? (expandIpv6(address) ?? address) : address
    if (!sans.includes(canonical)) sans.push(canonical)
  }
  add('::1')
  for (const entry of detectAddrs()) add(entry.address)
  if (state?.boundHost) add(state.boundHost)
  return sans
}

/** 保证证书存在、可解析且 SAN 覆盖当前网卡;
 *  缺失/损坏/SAN 漂移(如 Tailscale 等新网卡上线)时重建。
 *  返回 {key, cert, changed}。 */
async function ensureTlsCert(state) {
  const { dir, cert, key } = tlsAssetPaths()
  const readBoth = async () => {
    try {
      const [certText, keyText] = await Promise.all([readFile(cert, 'utf8'), readFile(key, 'utf8')])
      return { cert: certText, key: keyText }
    } catch {
      return null
    }
  }
  const existing = await readBoth()
  if (existing !== null) {
    try {
      const parsed = new X509Certificate(existing.cert)
      if (parsed.subject.includes('CN=dsh-lan-gate')) {
        const desired = desiredSanEntries(state)
        const have = sanEntriesOf(existing.cert)
        const drift = desired.length !== have.size || desired.some((entry) => !have.has(entry))
        if (!drift) return { ...existing, changed: false }
        // SAN 漂移 → 落到下方按当前网卡重建
      }
    } catch {
      /* 损坏 → 重新生成 */
  ...
  const pem = buildSelfSignedCert(desiredSanEntries(state))
  await mkdir(dir, { recursive: true })
  await writeFile(cert, pem.cert, { encoding: 'utf8' })
  await writeFile(key, pem.key, { encoding: 'utf8', mode: 0o600 })
  try {
    await chmod(key, 0o600)
  } catch {
    /* Windows 无意义,忽略 */
  }
  return { ...pem, changed: true }
}

/** 当前绑定模式希望暴露的局域网地址(与“核心服务实际绑到哪”解耦;
 *  null = local 模式,没有任何局域网面)。HTTP 关闭时核心只绑回环,
 *  HTTPS 仍按此地址监听。 */
function lanHostFor(state) {
  const mode = state.bind?.mode
  if (mode === 'all') return '0.0.0.0'
  if (mode === 'ip' && typeof state.bind?.ip === 'string') return state.bind.ip
  return null // 'local'
}

/** 当前应把 HTTPS 监听绑到哪个地址(null = 不应开启)。 */
function tlsDesired(state) {
  if (!(state.tls && state.tls.enabled === true)) return null
  return lanHostFor(state)
}

/** 让 HTTPS 监听与当前意图一致:多余的关掉,需要的开起来。 */
async function syncTlsServer(ctx, state) {
  const ws = ctx.get('webServer')
  if (!ws?.server) return { ok: false, error: 'webServer 不可用' }
  const desired = tlsDesired(state)
  const port = state.tls?.port ?? TLS_DEFAULT_PORT

  // 1) 目标变化/不再需要 → 先关旧监听(销毁其 socket,含 WSS 连接)
  if (state.tlsServer && (desired === null || state.tlsServer.host !== desired || state.tlsServer.port !== port)) {
    const old = state.tlsServer
    state.tlsServer = undefined
    await old.close()
  }
  if (desired === null) return { ok: true, active: false }

  // 2) 证书(每次同步检查 SAN 是否漂移;漂移则重建并热切换,无需重启)
  let creds
  try {
    creds = await ensureTlsCert(state)
  } catch (error) {
    ctx.logger.warn(`lan-gate: TLS 证书生成失败: ${String(error?.message ?? error)}`)
    return { ok: false, error: `TLS 证书生成失败: ${String(error?.message ?? error)}` }
  }
  if (state.tlsServer) {
    if (!creds.changed) return { ok: true, active: true }
    const old = state.tlsServer
    state.tlsServer = undefined
    await old.close()
  }

  // 3) HTTPS 监听(镜像 webServer 分发 + 网关;与 loopback twin 同构)
  let tls
  try {
    tls = createHttpsServer({ key: creds.key, cert: creds.cert, minVersion: 'TLSv1.2' }, twinRequestHandler(ws, ctx.logger))
    tls.on('upgrade', twinUpgradeHandler(ws, ctx.logger))
    installGateOnServer(ctx, state, tls, null)
    await listenServer(tls, port, desired)
  } catch (error) {
    if (tls) {
      releaseServerSockets(state, tls)
      try {
        tls.close()
      } catch {
        /* ignore */
    ...
    ctx.logger.warn(`lan-gate: HTTPS 监听 ${desired}:${port} 失败: ${String(error?.message ?? error)}`)
    return { ok: false, error: `HTTPS 监听失败: ${String(error?.message ?? error)}` }
  }
  state.tlsServer = {
    server: tls,
    host: desired,
    port,
    close: async () => {
      await closeServer(tls, state.socketSets.get(tls))
      releaseServerSockets(state, tls)
    }
  }
  ctx.logger.info(`lan-gate: HTTPS listening on https://${desired === '0.0.0.0' ? '<lan-ip>' : urlHost(desired)}:${port}`)
  return { ok: true, active: true }
}

/** IPv6 字面量放进 URL 需要方括号(IPv4/域名原样返回)。 */
function urlHost(host) {
  return host.includes(':') ? `[${host}]` : host
}

/** 当前 HTTPS 的可访问 URL 列表(供 status/UI 展示)。 */
function lanTlsUrls(state) {
  if (!state.tlsServer || state.tls?.enabled !== true) return []
  const port = state.tlsServer.port
  const host = lanHostFor(state)
  if (host === null) return []
  if (host === '0.0.0.0') return detectAddrs().map((entry) => `https://${urlHost(entry.address)}:${String(port)}`)
  return [`https://${urlHost(host)}:${String(port)}`]
}
...
function applyBindMode(ctx, state, mode, ip) {
  const run = async () => {
    ...
    // 明文 HTTP 是否暴露到局域网:关闭时核心服务只绑回环(电脑端 127.0.0.1 不受影响),
    // HTTPS 仍按 mode/ip 在局域网面监听(与核心绑定解耦,见 tlsDesired/lanHostFor)。
    const exposeHttp = state.http?.enabled !== false
    const coreHost = (!exposeHttp || target === '127.0.0.1') ? '127.0.0.1' : target
    ...
    // HTTPS 监听同样随绑定重建(地址/端口可能变化);结尾按需重新开启
    if (state.tlsServer) {
      const oldTls = state.tlsServer
      state.tlsServer = undefined
      await oldTls.close()
    ...
    try {
      ...
      await listenServer(server, port, coreHost)
    } catch (error) {
      ...
      state.boundHost = '127.0.0.1'
      ctx.logger.warn(`lan-gate: bind to ${coreHost} failed (${String(error?.message ?? error)}); staying on 127.0.0.1`)
      return { ok: false, error: `绑定 ${coreHost} 失败：${String(error?.message ?? error)}` }
    }
    state.boundHost = coreHost
    ...
    if (coreHost !== '0.0.0.0' && coreHost !== '127.0.0.1') {
      try {
        ...
        releaseServerSockets(state, twin)
        ctx.logger.warn(`lan-gate: loopback twin for ${coreHost} failed: ${String(error?.message ?? error)}`)
      }
    }
    ...
    // HTTPS 监听与绑定同步(绑到局域网面时按需开启/重建)
    const tlsOutcome = await syncTlsServer(ctx, state)
    if (!tlsOutcome.ok) ctx.logger.warn(`lan-gate: ${tlsOutcome.error ?? 'HTTPS 未开启'}`)
    ctx.logger.info(`lan-gate: bind switched ${wasBound} -> ${coreHost} (mode=${mode}, http=${exposeHttp})`)
    return { ok: true }
  }
 ...
 *  明文 HTTP 被关闭时返回空(即使绑定模式仍指向局域网面)。 */
function lanUrlsFor(state, port) {
  if (state.http?.enabled === false) return []
  const host = lanHostFor(state)
  if (host === null) return []
  if (host === '0.0.0.0') return detectAddrs().map((entry) => `http://${urlHost(entry.address)}:${String(port)}`)
...
async function handleAuthEndpoint(ctx, state, req, res, remote, local) {
  ...
  if (path === `${LAN_AUTH_PREFIX}/status` && method === 'GET') {
    ...
    sendJson(res, 200, {
      ...
      session: local ? true : hasValidSession(state, req),
      ...
      lanHost: lanHostFor(state),
      http: {
        enabled: state.http?.enabled !== false,
        active: state.boundHost !== '127.0.0.1'
      },
      lanUrls: lanUrlsFor(state, port),
      tls: {
        enabled: state.tls?.enabled === true,
        port: state.tls?.port ?? TLS_DEFAULT_PORT,
        active: state.tlsServer !== undefined
      },
      lanTlsUrls: lanTlsUrls(state)
    })
    return
  }
  ...
  if (path === `${LAN_AUTH_PREFIX}/http` && method === 'POST') {
    if (!local) {
      return sendJson(res, 403, { ok: false, error: { code: 'forbidden', message: '仅本机可修改 HTTP 设置' } })
    }
    let body
    try {
      body = await readBody(req)
    ...
    try {
      ...
      return sendJson(res, 400, { ok: false, error: { code: 'bad-request', message: '请求体不是 JSON' } })
    }
    state.http = { enabled: parsed.enabled === true }
    try {
      await saveStateFile(state)
    } catch (error) {
      ctx.logger.warn(`lan-gate: failed to persist http state: ${String(error)}`)
    }
    // 先应答再切换:重绑核心服务会短暂断开本连接(与 bind 切换相同的模式)
    sendJson(res, 200, { ok: true, accepted: true, switching: true, http: state.http })
    setTimeout(() => {
      applyBindMode(ctx, state, state.bind.mode, state.bind.ip).then((outcome) => {
        if (!outcome.ok) ctx.logger.warn(`lan-gate: http switch failed: ${outcome.error ?? 'unknown'}`)
      }).catch((error) => {
        ctx.logger.warn(`lan-gate: http switch threw: ${String(error)}`)
      })
    }, 80)
    return
  }

  if (path === `${LAN_AUTH_PREFIX}/tls` && method === 'POST') {
    if (!local) {
      return sendJson(res, 403, { ok: false, error: { code: 'forbidden', message: '仅本机可修改 HTTPS 设置' } })
    }
    let body
    try {
      body = await readBody(req)
    ...
    const port = parsed.port === undefined ? state.tls?.port ?? TLS_DEFAULT_PORT : Number(parsed.port)
    if (!Number.isInteger(port) || port < 1 || port > 65535) {
      return sendJson(res, 400, { ok: false, error: { code: 'bad-request', message: '端口无效(1-65535)' } })
    }
    const httpPort = ctx.get('webServer')?.port
    if (port === httpPort) {
      return sendJson(res, 400, { ok: false, error: { code: 'bad-request', message: 'HTTPS 端口不能与当前 HTTP 端口相同' } })
    }
    state.tls = { enabled: parsed.enabled === true, port }
    try {
      await saveStateFile(state)
    } catch (error) {
      ctx.logger.warn(`lan-gate: failed to persist tls state: ${String(error)}`)
    }
    const outcome = await syncTlsServer(ctx, state)
    sendJson(res, 200, {
      ok: true,
      tls: { enabled: state.tls.enabled, port: state.tls.port, active: state.tlsServer !== undefined },
      error: outcome?.ok ? undefined : (outcome?.error ?? 'HTTPS 未开启')
    })
    return
  }
...
/** Map a /mobile request path to a whitelisted asset name (null if unknown). */
function mobileAssetName(path) {
  if (path === MOBILE_PREFIX || path === `${MOBILE_PREFIX}/`) return 'index.html'
  const rest = path.slice(MOBILE_PREFIX.length + 1)
  return rest in MOBILE_ASSETS ? rest : null
}

/** Read one whitelisted mobile asset from this package (null if missing). */
async function readMobileAsset(name) {
  try {
    return await readFile(new URL(`../public/mobile/${name}`, import.meta.url), 'utf8')
  } catch {
    return null
  }
}

/** Serve one whitelisted /mobile asset (200) or a 404/500 JSON error. */
function serveMobileAsset(ctx, req, res, name) {
  readMobileAsset(name)
    .then((content) => {
      if (content === null) {
        if (apiish(req)) {
          sendJson(res, 404, { ok: false, error: { code: 'not-found', message: `mobile asset ${name} not found` } })
        } else {
      ...
      res.writeHead(200, { 'content-type': MOBILE_ASSETS[name], 'cache-control': 'no-store' })
      res.end(content)
    })
    .catch((error) => {
      ctx.logger.warn(`lan-gate: mobile asset ${name} failed: ${String(error)}`)
      try {
        if (!res.headersSent) {
          sendJson(res, 500, { ok: false, error: { code: 'internal', message: 'mobile asset failed' } })
        }
      } catch {
        /* ignore */
      }
...
function gateRequest(ctx, state, req, res, originals) {
  ...
  // /mobile: inert static shell — every caller may load it (without a session
  // the app shows its own login form, and every data call below /api still
  // answers 401/403). Authorized LAN clients also hit this branch.
  if (path === MOBILE_PREFIX || path.startsWith(`${MOBILE_PREFIX}/`)) {
    const asset = mobileAssetName(path)
    if (asset === null) {
      if (apiish(req)) {
        sendJson(res, 404, { ok: false, error: { code: 'not-found', message: `unknown mobile asset ${path}` } })
      } else {
        res.writeHead(404, { 'content-type': 'text/plain; charset=utf-8' })
        res.end('not found')
      }
    ...
    serveMobileAsset(ctx, req, res, asset)
    return
  }
...
export function apply(ctx) {
  ...
  const boot = async () => {
    ...
    try {
      ...
      if (loaded.tls) state.tls = loaded.tls
      if (loaded.http) state.http = loaded.http
    } catch (error) {
  ...
  ctx.effect(() => () => {
    ...
    if (state.tlsServer) {
      closeServer(state.tlsServer.server, state.socketSets.get(state.tlsServer.server))
      releaseServerSockets(state, state.tlsServer.server)
      state.tlsServer = undefined
    }
